...
void main()
{
	...
	void advanceNode(struct node **first);
...
void reverseList(struct node **first)
{
	...
	if(current->ptr!= NULL)
	{
		...
		printf("\n\t\tList Reversed .... Press Any Key....");
	}
	else
		printf("\n\t\tNot Enough Nodes To Reverse .... Press Any Key....");
}

void advanceNode(struct node **first)
{
	struct node *curr1,*prev1,*curr2;
	int nodeValue1,nodeValue2,count;
	curr1=prev1=*first;
	if((*first==NULL)||((*first)->ptr==NULL))
	{
		printf("\n\t\tNot Enough Nodes To Move .... Press Any Key....");
		return;
	}
	printf("\n\nAvailable Nodes Are : \n");
	display(*first,'n');
	...
	fflush(stdin);
	while((scanf("%d",&nodeValue1)==0)||(nodeValue1<=0))
	{
		fflush(stdin);
		printf("\n\nEnter VALID First Node Value: ");
	}
	...
	for(count=1;count<nodeValue1&& curr1!=NULL;count++)
	{
		prev1=curr1;
		curr1=curr1->ptr;
	}
	...
	{
		printf("\n\nFIRST NODE NOT FOUND ");
		printf("\n\t\tNode Cannot Be Moved .... Try Again Later ....Press Any Key....");
		return;
	}

	printf("\n\nEnter Second Node Number : ");
	fflush(stdin);
	while((scanf("%d",&nodeValue2)==0)||(nodeValue2<=0))
	{
		fflush(stdin);
		printf("\n\nEnter VALID Second Node Value: ");
	}
	...
	for(count=0;count<nodeValue2&& curr2!=NULL;count++)
	{
		curr2=curr2->ptr;
	}
	if(curr2==NULL)
	...
	printf("\n\t\tNodes Found!!!!REALLY MOVE?('y' to Move) ");
	fflush(stdin);
	if(_getche()=='y')
	{
		if(curr1==prev1)
